...
namespace API
{
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IConfiguration configuration, ExilenceContext exilenceContext)
        {
            ...
            exilenceContext.Database.ExecuteSqlRaw($"DELETE FROM Connections WHERE InstanceName = '{instanceName}'"); //Remove faulty connections to this node on startup if node crasched
            exilenceContext.Database.ExecuteSqlRaw($"delete from Groups where Id in (select g.Id from Groups g where (select count(*) from Connections where GroupId = g.Id) = 0)"); //Remove groups with no connections after connection cleanup
        }
    }
}
